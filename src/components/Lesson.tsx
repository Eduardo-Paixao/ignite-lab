...
export const Lesson = ({ title, type, availableAt, slug }: LessonProps) => {
  const { slugUrl } = useParams<{ slugUrl: string }>();
  const isActiveLesson = slug === slugUrl;
  const isAvailableAt = isPast(availableAt);
    ...
    <Link to={`/event/lesson/${slug}`} className="group">
      ...
      >
        <header className="flex items-center justify-between">
          ...
          <span className={`text-xs rounded py-[0.125rem] px-2 text-white border ${isActiveLesson?'border-white':'border-green-300'} font-bold`}>
